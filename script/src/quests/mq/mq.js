$.Module(function (App) {
    ...
    let reQuest = /^([^：()\[\]]{2,5})对你说道：我早就看(.*)不顺眼，听说他最近在(.*)，你去做了他/
            ...
            task.AddTrigger(reStart, (tri, result) => {
                if (mode == 4) {
                    if (name) {
                        ...
                        task.Data.SetZone(result[1].slice(0, 2), true)
                        return false
                    }
                }
                return true
    ...
    MQ.GoAskInfo = function () {
        if (MQ.Data.NPC.Info.length) {
            let info = MQ.Data.NPC.Info.shift()
            let infoid = ""
            if (info != null) {
                $.PushCommands(
                    $.To(info),
